...
//-------------------------------------------------------------------------
//QuÈ significa la v de 'glColor4dv(value_ptr(mColor));'
//Para quÈ sirve value_ptr();
//-------------------------------------------------------------------------
...
void TrianguloRGB::update() {
	//Coordenadas de traslaciÛn
	double x = 0;
	double y = 0;

	anguloDesplazamiento++;
	//Actualizamos las coordenadas de traslaciÛn respecto al radio del cÌrculo (100) y el angulo de desplazamiento
	x = 200 * cos(radians(anguloDesplazamiento));
	y = 200 * sin(radians(anguloDesplazamiento));


	//Primero se realiza la traslaciÛn con la matriz identidad 
	setModelMat(translate(dmat4(1), dvec3(x, y, 0.0)));
	//DespuÈs se rota utilizando la propia matriz (para que rote sobre si mismo)
