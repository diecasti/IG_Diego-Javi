...
glm::dvec3 IndexMesh::CalculoVectorNormalPorNewell(GLuint* face) {
	...
	for (int i = 0; i < 2; i++) {
		vertActual = vVertices[face[i]];	//Accedemos al vertice que se corresponde a cada Ã­ndice
...
Grid* Grid::generateGrid(GLdouble lado, GLuint nDiv)
{
	...
	GLuint numFC = nDiv + 1; // nÃºmero de vÃ©rtices por filas y columnas
	...
	// GeneraciÃ³n de Ã­ndices
	mesh->nNumIndices = nDiv * nDiv * 6;
	mesh->vIndices = new GLuint[mesh->nNumIndices];


	int i = 0; // array de Ã­ndices
...
Grid* Grid::generaGridTex(GLdouble lado, GLuint numDiv) {
	...
	GLuint numFC = numDiv + 1; // nÃºmero de vÃ©rtices por filas y columnas
	// GeneraciÃ³n de las coordenadas de textura
	mesh->vTexCoords.reserve(mesh->mNumVertices);
